...
public class ProfileServiceImpl implements ProfileService {
    ...
    public void sendResetOtp(String email) {
        ...
        user.setResetOtpExpireAt(System.currentTimeMillis() + (5 * 60 * 1000)); // 5 minutes
    ...
    public void resetPassword(String email, String otp, String newPassword) {
        ...
        if (attempts >= 3) {
            throw new RuntimeException("Max OTP attempts reached. Please resend OTP.");
        }
        ...
        if (!otp.equals(user.getResetOtp())) {
            ...
            userRepo.save(user);
            throw new RuntimeException("Invalid OTP. Attempts: " + (attempts + 1) + "/3");
        }
    ...
    public void sendOtp(String email) {
        ...
        user.setVerifyOtpExpireAt(System.currentTimeMillis() + (5 * 60 * 1000)); // 5 minutes
        user.setOtpAttempts(0);
    ...
    @Override
    public ProfileResponse createAdminProfile(ProfileRequest request) {
        if (userRepo.existsByEmail(request.getEmail())) {
        ...
        UserEntity newUser = UserEntity.builder()
                ...
                .password(passwordEncoder.encode(request.getPassword()))
                .role(Role.ADMIN)
                .isAccountVerified(true) // Admins auto-verified for simplicity, or false if OTP needed
                .build();
    ...
    public boolean isAccountLocked(String email) {
        ...
        long LOCK_DURATION = 1000 * 60 * 15; // 15 minutes
        if (System.currentTimeMillis() > user.getLockTime() + LOCK_DURATION) {
            user.setLockTime(null);
            user.setFailedLoginAttempts(0);
            userRepo.save(user);
            return false;
